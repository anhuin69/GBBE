class Storage < ActiveRecord::Base
  belongs_to :user
  has_many :items
  has_one :root, -> (storage){ where remote_id: Gatherbox::Application.config.api[storage.provider][:ROOT]}, class_name: 'Item'
  ...
  def as_json(options)
    super(:only => [:id, :provider, :login, :quota_bytes_total, :quota_bytes_used, :etag])
  end
